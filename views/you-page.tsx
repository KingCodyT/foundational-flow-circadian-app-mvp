...
export default function YouPage() {
    ...
    () =>
      Object.entries(personalization.perSignal)
        .filter(([id, signal]) => {
          ...
          return (
            definition?.classification === SignalClassification.BEHAVIOR &&
            Boolean(signal.coachingState)
          );
        })
        .map(([id, signal]) => ({
          id,
  ...
  const groupedSignals = {
    needsAttention: behavioralSignals.filter(
      (signal) => signal.state === CoachingState.NEEDS_ATTENTION
    ),
    developing: behavioralSignals.filter(
      (signal) => signal.state === CoachingState.DEVELOPING
    ),
    established: behavioralSignals.filter(
      (signal) => signal.state === CoachingState.ESTABLISHED
    ),
  };

  const targetSignal = primaryTarget.signalId
    ? personalization.perSignal[primaryTarget.signalId]
    : null;
  const targetDefinition = primaryTarget.signalId
    ? SIGNAL_REGISTRY[primaryTarget.signalId]
    : null;
  ...
  return (
    <FlowShell>
      <section className="mx-auto max-w-4xl">
        ...
        <h1 className="mt-4 font-[family-name:var(--font-display)] text-4xl tracking-[-0.03em] sm:text-5xl">
          What your biology is telling us.
        </h1>

        <p className="mt-4 max-w-2xl text-lg leading-8 text-[var(--color-muted)]">
          Foundational Flow learns your patterns so it can coach what matters, adapt when life changes, and get quieter when a signal is handled.
        </p>
        ...
        <div className="mt-10 rounded-3xl border border-[var(--color-line)] bg-white/70 p-6 sm:p-8">
          ...
          <div className="mt-6 grid gap-x-8 gap-y-6 sm:grid-cols-2 lg:grid-cols-3">
            ...
            <ProfileFact
              label="Sleep window"
              value={formatProfileTime(dailyProfile?.targetBedtime)}
            />
            <ProfileFact
              label="Guidance level"
              value={participationLabel(participationLevel)}
            />
            <ProfileFact
              label="Sunrise"
              value={formatSolarTime(environment.sunrise)}
            />
            <ProfileFact
              label="Sunset"
              value={formatSolarTime(environment.sunset)}
            />
            <ProfileFact
              label="Day length"
              value={formatDayLength(environment.dayLengthMinutes)}
            />
          </div>
          ...
          {hasCompletedAudit && targetDefinition && primaryTarget.signalId ? (
            <>
              <div className="mt-4 flex flex-wrap items-center gap-3">
                ...
                <span className="rounded-full border border-[var(--color-gold)] px-3 py-1 text-xs font-semibold uppercase tracking-[0.12em]">
                  {coachingLabel(primaryTarget.coachingState)}
                </span>
              </div>

              <p className="mt-4 max-w-2xl leading-7 text-[var(--color-muted)]">
                {hierarchyExplanation(primaryTarget.hierarchy)}
              </p>
            ...
            <>
              <h2 className="mt-4 text-2xl font-semibold">
                No active correction is demanding attention.
              </h2>
              <p className="mt-3 max-w-2xl leading-7 text-[var(--color-muted)]">
                That does not mean the system is finished learning. It means there is not enough reason to interrupt you with a primary target right now.
              </p>
            </>
          ) : (
            <>
              <h2 className="mt-4 text-2xl font-semibold">
                I need your starting signal first.
              </h2>
              <p className="mt-3 max-w-2xl leading-7 text-[var(--color-muted)]">
                Complete the circadian assessment and this section will show the first biological target worth coaching.
              </p>
            </>
          )}
        </div>
          ...
          <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
            <div>
              ...
              <h2 className="mt-3 text-2xl font-semibold">
                What still needs coaching — and what can stay quiet.
              </h2>
            </div>
          </div>
          ...
          {behavioralSignals.length > 0 ? (
            <div className="mt-7 grid gap-4 lg:grid-cols-3">
              <FoundationGroup
                ...
                description="Worth actively coaching now."
                signals={groupedSignals.needsAttention}
              />
              <FoundationGroup
                title="Developing"
                description="Improving. Coaching can begin to back off."
                signals={groupedSignals.developing}
              />
              <FoundationGroup
                title="Established"
                description="Supported enough to mostly stay in the background."
                signals={groupedSignals.established}
              />
            </div>
          ) : (
            <p className="mt-6 leading-7 text-[var(--color-muted)]">
              There is not enough behavioral evidence yet to map your foundations. The app will fill this in as it learns you.
            </p>
          )}
        </div>
      </section>
...
function FoundationGroup({
  ...
  title: string;
  description: string;
  signals: { id: string; label: string; state?: CoachingState }[];
}) {
  return (
